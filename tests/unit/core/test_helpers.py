...
def test_swallowing_filter_with_none():
    assert _swallowing_filter(None) is None


def test_swallowing_filter_passes_value_through():
    fn = _swallowing_filter(lambda req: "admin")
    assert fn(_mock_request()) == "admin"


def test_swallowing_filter_returns_none_when_callback_raises():
    def cb(req: Request) -> str:
        raise AssertionError("no session on this request")

    fn = _swallowing_filter(cb)
    assert fn(_mock_request()) is None
