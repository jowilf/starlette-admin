...
class TestBeanieRelations:
    ...
    async def test_relationships_base(self, client):
        ...
        response = await client.get(f"/admin/store/detail?pk={store.id}")
        assert response.status_code == 200, response.text

        # check select2
        response = await client.get("/admin/_api/store/relation-lookup")
        assert response.status_code == 200, response.text
    ...
    async def test_edit_relationships_list(self, client):
        ...
        response = await client.post(
            f"/admin/store/edit?pk={store.id}",
            data={
                "name": "Jewelry store",
