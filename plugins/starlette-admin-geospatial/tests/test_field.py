def test_field_template_resolves_by_plain_and_prefixed_path(admin):
    plain = admin.templates.env.get_template(
        "plugins/geospatial/fields/form/rating.html"
    )
    prefixed = admin.templates.env.get_template("@geospatial/fields/form/rating.html")
...
def test_create_form_renders_rating_widget(client):
    ...
    assert "plugins/geospatial/css/rating.css" in response.text
    assert "plugins/geospatial/js/rating.js" in response.text
