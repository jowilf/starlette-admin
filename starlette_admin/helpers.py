...
def _swallowing_filter(
    fn: Callable[[Request], Any] | None,
) -> Callable[[Request], Any] | None:
    """Wrap a template filter so provider errors become ``None``.

    The layout template calls ``get_admin_user``/``get_admin_config`` on every
    render, including renders that happen outside an authenticated request (the
    exception handler drawing ``error.html`` for a missing static asset before
    the auth redirect fires, see #754).  Letting the provider raise there turns
    a 404 into a 500, so a failing lookup is treated as "no user".
    """
    if fn is None:
        return None

    def _wrapped(request: Request) -> Any:
        try:
            return fn(request)
        except Exception:
            _log.debug("template filter %r failed; rendering without it", fn)
            return None
