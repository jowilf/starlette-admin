$(function () {
    ...
    */
    ...
    (function () {
        ...
        while (fringe.length > 0) {
            ...
            if (field.type === "CollectionField") fringe = field.fields
                .map((f) => {
                ...
                .concat(fringe); else if (field.type === "ListField") {
    ...
    var actionManager = new ActionManager(model.actionUrl, model.rowActionUrl, function (query, element) {
        // appendQueryParams
        if (element.data("is-row-action") === true) query.append("pk", element.closest(".row-actions-container").data("id")); else selectedRows.forEach((s) => {
            query.append("pks", s);
        });
    ...
    buttons = [];
    export_buttons = [];
    if (model.exportTypes.includes("csv")) export_buttons.push({
        extend: "csv", text: function (dt) {
            return `<i class="fa-solid fa-file-csv"></i> ${dt.i18n("buttons.csv")}`;
        }, exportOptions: {
            columns: model.exportColumns, orthogonal: "export-csv",
        },
    });
    if (model.exportTypes.includes("excel")) export_buttons.push({
        extend: "excel", text: function (dt) {
            return `<i class="fa-solid fa-file-excel"></i> ${dt.i18n("buttons.excel")}`;
        }, exportOptions: {
            columns: model.exportColumns, orthogonal: "export-excel",
        },
    });
    if (model.exportTypes.includes("pdf")) export_buttons.push({
        extend: "pdf", text: function (dt) {
            return `<i class="fa-solid fa-file-pdf"></i> ${dt.i18n("buttons.pdf")}`;
        }, exportOptions: {
            columns: model.exportColumns, orthogonal: "export-pdf",
        },
    });
    if (model.exportTypes.includes("print")) export_buttons.push({
        extend: "print", text: function (dt) {
            return `<i class="fa-solid fa-print"></i> ${dt.i18n("buttons.print")}`;
        }, exportOptions: {
            columns: model.exportColumns, orthogonal: "export-print",
        },
    });
    if (export_buttons.length > 0) buttons.push({
        extend: "collection", text: function (dt) {
            return `<i class="fa-solid fa-file-export"></i> ${dt.i18n("starlette-admin.buttons.export")}`;
        }, className: "", buttons: export_buttons,
    });
    ...
    if (model.columnVisibility) buttons.push({
        extend: "colvis", text: function (dt) {
            return `<i class="fa-solid fa-eye"></i> ${dt.i18n("buttons.colvis")}`;
        },
    });

    if (model.searchBuilder) buttons.push({
        extend: "searchBuilder", config: {
            columns: model.searchColumns, conditions: {
                bool: {
    ...
    */
    function extractCriteria(c) {
        ...
        if ((c.logic && c.logic == "OR") || c.logic == "AND") {
            ...
            if (c.type.startsWith("moment-")) {
                searchFormat = dt_fields.find((f) => f.name == c.origData)?.search_format;
    ...
    var table = $("#dt").DataTable({
        ...
        select: {
            style: "multi", selector: "td:first-child .form-check-input", className: "row-selected",
        },
        language: {
            url: model.dt_i18n_url, infoFiltered: "", select: {
                rows: {
                    0: "",
                },
            }, searchBuilder: {
        ...
        ajax: function (data, callback, settings) {
            ...
            query = {
                skip: settings._iDisplayStart, limit: settings._iDisplayLength, order_by: order,
            };
            if (data.search.value != "") query.where = data.search.value; else if (where) query.where = JSON.stringify(where);
            $.ajax({
                url: model.apiUrl,
                type: "get",
                data: query,
        ...
        columns: [{
            data: "DT_RowId", orderable: false, checkboxes: {
                selectRow: true, selectAllRender: '<input class="form-check-input dt-checkboxes" type="checkbox">',
            }, render: render.col_0,
        }, {
        ...
        stateSaveCallback: function (settings, data) {
            const params = {
                ...
                "order": data?.order ? data.order
                    .map(([col, dir]) => `${dir === 'asc' ? '' : '-'}${dt_columns[col - 2].name}`) : undefined,
                "searchBuilder": data?.searchBuilder && !$.isEmptyObject(data?.searchBuilder) ? JSON.stringify(data?.searchBuilder) : undefined,
            };
        ...
        stateLoadCallback: function (settings, callback) {
            ...
            let length = isNaN(parseInt(params?.page_size)) ? model.pageSize : parseInt(params?.page_size)
            ...
            if (params?.order) {
                ...
                params.order.forEach((o) => {
                    ...
                    const colName = o.startsWith('-') || o.startsWith('+') ? o.substring(1) : o;
            ...
            var state = {
                ...
                "search": {
                    "search": params?.search ?? undefined, "smart": true, "regex": false, "caseInsensitive": true
                },
                "searchBuilder": params?.searchBuilder ? JSON.parse(params?.searchBuilder) : undefined,
                "page": page,
                "start": length * page,
            };
    ...
    function onSelectChange() {
        ...
        if (table.rows({selected: true}).count() == 0) $("#actions-dropdown").hide(); else $("#actions-dropdown").show();
