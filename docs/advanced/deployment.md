...
Gunicorn's `uvicorn.workers.UvicornWorker` reads the same two settings from `--forwarded-allow-ips`. See [Uvicorn's deployment docs](https://www.uvicorn.org/deployment/) for the full set of process-manager options.
