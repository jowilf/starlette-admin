---
title: Расширяемые интерфейсы администрирования для FastAPI и Starlette
description: Создавайте полноценный интерфейс администрирования на основе ваших моделей
  SQLAlchemy, SQLModel, Beanie, MongoEngine или Tortoise ORM.
keywords:
- fastapi admin
- starlette admin
- python admin panel
...
<div class="home-hero">
  ...
  <h1 class="home-title">Расширяемые <span class="home-gradient">интерфейсы администрирования</span><br>для FastAPI и Starlette</h1>
  <p class="home-sub">Создавайте полноценный интерфейс администрирования на основе ваших моделей SQLAlchemy, SQLModel, Beanie, MongoEngine или Tortoise ORM. Работая на базе <a href="https://tabler.io">UI-кита Tabler</a>, starlette-admin предоставляет страницы списка, автоматически сгенерированные формы, экспорт данных и надёжную аутентификацию. Настройте весь интерфейс на Python, не написав ни строчки фронтенд-кода.</p>
  <div class="home-actions">
    <a class="md-button md-button--primary home-btn" href="getting-started/quickstart/">Начать работу</a>
    <a class="md-button home-btn" href="https://starlette-admin-demo.jowilf.com/">Онлайн-демо</a>
  ...
  <div class="home-shot">
    <img src="assets/images/list-preview.png" alt="дашборд starlette-admin со статистическими виджетами, таблицами недавней активности и боковой панелью представлений моделей" loading="lazy">
  </div>
</div>

<h2 class="home-section-title">Встроенные возможности</h2>
<p class="home-lede">Всё необходимое работает сразу после установки. Для каждой базовой возможности предусмотрены документированные точки расширения, так что вы сможете адаптировать её под свои требования.</p>
...
<div class="home-cards">
  <a class="home-card" href="user-guide/views/">
    ...
    <h3>Таблицы</h3>
    <p>Просматривайте, ищите и сортируйте данные с постраничной навигацией, сортировкой по нескольким столбцам и URL, сохраняющими состояние. Редактируйте поля в режиме инлайн-редактирования прямо на странице списка.</p>
  </a>
  <a class="home-card" href="user-guide/filters/">
    ...
    <h3>Фильтры</h3>
    <p>Стройте вложенные запросы AND/OR прямо в интерфейсе, с операторами, учитывающими тип данных: текст, числа, даты и логические значения.</p>
  </a>
  <a class="home-card" href="user-guide/fields/">
    ...
    <p>Автоматически создавайте формы более чем для 25 типов полей и для связанных данных. Отправляйте загружаемые файлы в локальное хранилище или S3.</p>
  </a>
  <a class="home-card" href="user-guide/actions/">
    ...
    <h3>Действия</h3>
    <p>Создавайте групповые операции и операции над строками с помощью стандартных декораторов Python. Подтверждайте каждый запуск через модальные окна и пользовательские формы параметров.</p>
  </a>
  <a class="home-card" href="user-guide/export-import/">
    ...
    <p>Экспортируйте записи в CSV, Excel, JSON, PDF или любой формат, поддерживаемый tablib. Импортируйте данные пакетно с помощью мастера с предварительным просмотром, который проверяет каждую строку перед записью в базу данных.</p>
  </a>
  <a class="home-card" href="user-guide/auth/">
    ...
    <p>Подключите провайдер аутентификации, которым уже пользуетесь. Развёртывайте с настройками по умолчанию, готовыми к продакшену: защита от CSRF и встроенные ограничения на экспорт и импорт.</p>
  </a>
  <a class="home-card" href="user-guide/inline-forms/">
    ...
    <h3>Инлайн-формы</h3>
    <p>Управляйте связанными данными на месте. Редактируйте дочерние записи внутри формы родительской модели, не покидая страницу.</p>
  </a>
  <a class="home-card" href="user-guide/custom-views/">
    ...
    <h3>Дашборды</h3>
    <p>Соберите главную страницу из встроенных виджетов статистики, графиков и таблиц либо замените её полностью пользовательским представлением.</p>
  </a>
  <a class="home-card" href="user-guide/i18n/">
    ...
    <p>Предоставляйте панель администрирования на нескольких языках с форматированием с учётом локали и точной обработкой часовых поясов из коробки.</p>
  </a>
</div>
...
<div class="home-code-head">
  ...
  <h2 class="home-section-title">Всё на Python</h2>
  <p class="home-lede">Создавайте полноценный интерфейс администрирования с помощью чистого Python API, спроектированного для <strong>быстрой разработки</strong>, <strong>читаемого синтаксиса</strong> и <strong>долгосрочной сопровождаемости</strong>.</p>
</div>

=== "Подключение панели"

    <span class="home-tour-title" role="heading" aria-level="3">Подключите панель администрирования</span>

    Зарегистрируйте модель и подключите панель администрирования к любому приложению FastAPI или Starlette. Затем выполните `fastapi dev` и откройте `/admin`.
...
=== "Поля"
    ...
    Переопределяйте любое автоматически определённое поле, чтобы управлять валидацией, видимостью на каждой странице и тем, как starlette-admin считывает и отображает значения.
...
=== "Фильтры"
    ...
    Расширяйте встроенный конструктор запросов пользовательскими фильтрами, соответствующими вашим бизнес-правилам. Нужные операции можно применять напрямую к модели базы данных.
...
=== "Действия"
    ...
    Добавляйте бизнес-операции одним декоратором. Модальные окна подтверждения, пользовательские формы и флеш-сообщения уже встроены во фреймворк.
...
=== "Аутентификация"
    ...
    Реализуйте три стандартных метода вокруг собственной проверки учётных данных. starlette-admin берёт на себя страницу входа, сессии и перенаправления.
...
=== "Дашборд"
    ...
    Соберите главную страницу панели из виджетов статистики, графиков и таблиц, которые запрашивают актуальные данные при каждом обращении.
...
<p class="home-lede">Каждый слой можно заменить. Упаковывайте функциональность в самодостаточные плагины или подключайтесь к специальным точкам расширения, чтобы адаптировать фреймворк под свою предметную область.</p>
...
<p>Установите пакет плагина и передайте его экземпляру <code>Admin</code>. Поля, конвертеры, шаблоны и статические ресурсы подключаются автоматически.</p>
    ...
    <a class="home-more" href="advanced/plugins/">Руководство по плагинам</a>
  </div>
  <div class="home-plugin-panel">
    <span class="home-eyebrow hc-emerald">Точки расширения</span>
    <h3>Подключайтесь к любому компоненту</h3>
    <p>Предопределённые интерфейсы позволяют заменять или расширять каждую часть независимо. Создайте подкласс нужного базового класса и зарегистрируйте его. Можно настроить всё: от процесса аутентификации до форматов экспорта.</p>
    <ul class="home-hooks">
      ...
      <li><a href="user-guide/export-import/"><span>Экспортёры</span><code>BaseExporter</code></a></li>
      <li><a href="user-guide/export-import/"><span>Импортёры</span><code>BaseImporter</code></a></li>
      ...
      <li><a href="user-guide/file-storage/"><span>Бэкенды хранилища</span><code>BaseStorage</code></a></li>
    ...
    <a class="home-more" href="advanced/extension-points/">Изучите все точки расширения</a>
  </div>
</div>
